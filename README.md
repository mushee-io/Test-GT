...
Deploy this repo to Vercel as **two projects**. This repo is a monorepo with a separate Express server and Vite client, so the clean Vercel setup is:
...
## If the backend shows “This Serverless Function has crashed”

The server should no longer hard-crash on a missing wallet address. Visit `/` or `/health` on the backend first.

- If `/` returns JSON with `startupErrors`, one or more required environment variables are missing.
- If `/health` returns `{ "ok": false }`, fix the missing variables in Vercel Project Settings and redeploy.
- The backend project should use **Root Directory** = `apps/server`.
